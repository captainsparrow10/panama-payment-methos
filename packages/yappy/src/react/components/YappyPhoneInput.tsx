 ...
 * @returns `true` if the phone number is valid for Yappy, `false` otherwise.
 *
 * @example
 * ```ts
...
/**
 * Props for the `YappyPhoneInput` component.
 *
 * @example
 * ```tsx
 * <YappyPhoneInput
...
export interface YappyPhoneInputProps {
  ...
  placeholder?: string;
  /** Label text. Defaults to "Numero de telefono Yappy". */
  label?: string;
 ...
 * @returns A form element with phone input, validation, and submit button.
 *
 * @example
 * ```tsx
...
export function YappyPhoneInput({
  ...
  className,
  placeholder = 'Ej: 60800011',
  label = 'Numero de telefono Yappy',
    ...
    <form onSubmit={handleSubmit} className={className} noValidate>
      <fieldset disabled={disabled}>
        ...
        <p id="yappy-phone-hint" className="sr-only">
          Ingresa tu numero de telefono sin el codigo +507. Ejemplo: 60800011
        </p>

        {showError && (
          <p id="yappy-phone-error" role="alert" aria-live="polite">
            El numero debe tener 8 digitos y empezar con 6 o 7. Ejemplo: 60800011
          </p>
        )}
