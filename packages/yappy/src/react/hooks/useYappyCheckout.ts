...
import type { YappyCheckoutResult } from '../../types.js';

/**
 * Configuration for the `useYappyCheckout` hook.
 *
 * @example
 * ```ts
 * const config: UseYappyCheckoutConfig = {
...
/**
 * Return type of the `useYappyCheckout` hook.
 *
 * @example
 * ```ts
 * const { initPayment, isLoading, error, data, reset } = useYappyCheckout(config);
 * ```
 */
export interface UseYappyCheckoutReturn {
   ...
   * @example
   * ```ts
   * await initPayment('60800011', { total: '25.00', cartId: 'abc' });
   * ```
   */
   ...
   * @example
   * ```ts
   * reset(); // Clear state to allow a new payment attempt
   * ```
   */
  reset: () => void;
}
