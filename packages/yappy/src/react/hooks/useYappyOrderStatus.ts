...
import { YappyStatus } from '../../types.js';
 ...
 * @example
 * ```ts
 * const data: YappyOrderStatusData = {
 *   status: 'paid',
 *   errorMessage: null,
 *   order: { id: '123', total: 25.00 },
 * };
 * ```
 */
...
/**
 * Configuration for the `useYappyOrderStatus` hook.
 *
 * @example
 * ```ts
 * const config: UseYappyOrderStatusConfig = {
...
/**
 * Return type of the `useYappyOrderStatus` hook.
 *
 * @example
 * ```ts
 * const { data, isPolling, error, stopPolling, startPolling } = useYappyOrderStatus(config);
 * ```
 */
 ...
 *   }, [data?.status]);
 *
 *   return <div>{isPolling ? 'Esperando confirmacion...' : data?.status}</div>;
 * }
 * ```
 */
