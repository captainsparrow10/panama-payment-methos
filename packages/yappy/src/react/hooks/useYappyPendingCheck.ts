 ...
 *
 * EXPIRATION WARNING: Yappy payment orders expire after exactly 5 minutes.
 ...
 *
 * REFRESH RECOVERY: This hook saves the pending order to localStorage under
 ...
 * @example
 * ```ts
 * const status: YappyPendingStatus = 'pending';
 * if (status === 'paid') { redirectToSuccess(); }
 * ```
 */
 ...
 * @example
 * ```ts
 * const order: YappyPendingOrderData = {
 *   orderId: 'A3KX9MZQ1BPRY7W',
...
/**
 * Configuration for the `useYappyPendingCheck` hook.
 *
 * @example
 * ```ts
 * const config: UseYappyPendingCheckConfig = {
...
/**
 * Return type of the `useYappyPendingCheck` hook.
 *
 * @example
 * ```ts
 * const {
 *   status, timeLeft, pendingOrder,
 *   startPayment, cancelPayment, reset,
 *   isLoading, error,
...
export interface UseYappyPendingCheckReturn {
   ...
   * @example
   * ```ts
   * await startPayment('60800011', { total: '25.00', cartId: 'abc' });
   * ```
   */
   ...
   * @example
   * ```ts
   * reset(); // Clear all state and stop polling/timer
   * ```
   */
  reset: () => void;
...
/**
 * Persists pending order data to localStorage.
 *
 * @param data - The order data to persist.
 *
 * @example
 * ```ts
 * savePending({ orderId: 'ABC', transactionId: 'TXN', expiresAt: '...', createdAt: '...' });
 * ```
 */
...
/**
 * Loads pending order data from localStorage.
 *
 * @returns The pending order data, or null if none exists.
 *
 * @example
 * ```ts
 * const saved = loadPending();
 * if (saved) { resumePolling(saved); }
 * ```
 */
...
/**
 * Clears pending order data from localStorage.
 *
 * @example
 * ```ts
 * clearPending(); // Remove stored pending order
 * ```
 */
 ...
 *
 * Manages checkout initiation, status polling, countdown timer, localStorage
 * persistence, and terminal state callbacks. This is the all-in-one hook for
 * building a fully custom Yappy payment UI.
 *
