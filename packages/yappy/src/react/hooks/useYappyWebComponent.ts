 ...
 *   });
 *
 *   if (!isOnline) return <p>Yappy no esta disponible en este momento.</p>;
 *
 ...
 * @example
 * ```ts
 * const btn = document.querySelector('btn-yappy') as BtnYappyElement;
 * btn.eventPayment({ transactionId: '...', token: '...', documentName: '...' });
 * ```
 */
...
/**
 * Configuration for the `useYappyWebComponent` hook.
 *
 * @example
 * ```ts
 * const config: UseYappyWebComponentConfig = {
 *   checkoutEndpoint: '/api/yappy/checkout',
 *   checkoutPayload: { cartId: 'abc123' },
...
/**
 * Return type of the `useYappyWebComponent` hook.
 *
 * @example
 * ```ts
 * const { btnRef, isOnline, isLoading, isCdnLoaded, setLoading } = useYappyWebComponent(config);
 * ```
 */
 ...
 *     onError: (detail) => showToast('Payment failed'),
 *     environment: 'sandbox',
 *   });
 *
 *   if (!isOnline) return <p>Yappy no disponible.</p>;
 *   if (!isCdnLoaded) return <p>Cargando...</p>;
 *
 *   return <btn-yappy ref={btnRef} theme="blue" rounded="true" />;
 * }
 * ```
