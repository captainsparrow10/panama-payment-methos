...
import type { YappyButtonConfig, YappyButtonTheme } from '../types.js';
 ...
 * @example
 * ```ts
 * const btn = container.querySelector('btn-yappy') as BtnYappyElement;
 * btn.eventPayment({ transactionId: '...', token: '...', documentName: '...' });
 * ```
 */
interface BtnYappyElement extends HTMLElement {
  /** Triggers the Yappy payment flow with the checkout params. */
  eventPayment: (params: { transactionId: string; token: string; documentName: string }) => void;
  /** Controls the button's visual loading state. */
  isButtonLoading: boolean;
}

/**
 * Configuration options for `initYappyButton()`.
 *
 * @example
 * ```ts
 * const options: InitYappyButtonOptions = {
